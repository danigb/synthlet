...
- `frequency`: the frequency of the filter in Hz. Default is 1000Hz. Range: [20-20000]
- `Q`: the resonance of the filter. Default is 0.7071 (Butterworth: maximally flat, -3dB at the cutoff). Range: [0.025-40]
- `gain`: the shelf or bell gain in dB. Only `Bell`, `LowShelf` and `HighShelf` use it; for every other type it is inert. Default is 0, which is an exact bypass. Range: [-40-40]

On the three types that use `gain`, `Q` sets the resonance of the _corner_. At or below 0.7071 the response stays inside the requested gain; above it the corner peaks, which is what a resonant shelf is for — a +12dB high shelf at Q=40 reaches about +41dB at its corner.
