...
const audioContext = await registerAllWorklets(new AudioContext());
const s = getSynthlet(audioContext);

// Create the parameters you want to control
const gate = s.param();
const filterFreq = s.param(1500);
// Create and connect the modules
const out = s.conn(
  s.polyblep.saw(440),
  s.svf.lp(filterFreq),
