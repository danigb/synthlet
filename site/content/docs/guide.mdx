...
3. **Own what you built and publish what you expose, with
   `disposable(out, [...], { ... })`.** Anything you passed to a factory is
   already owned by the module you passed it to; list the nodes you connected by
   hand. Listing extras is free - `dispose()` runs once per node - so when in
   doubt, list everything you created. The third argument is the compound's
...
function Voice(ac: AudioContext, frequency = 440) {
  ...
  return disposable(out, [osc, filterEnv, filter, amp, gate, volume], {
    gate: gate.input,
    volume: volume.input,
    osc,
    filter,
