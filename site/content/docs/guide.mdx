...
thing down. There is no special API for building one - four rules:
...
3. **Own what you built, with `disposable(out, [...])`.** Anything you passed to
   a factory is already owned by the module you passed it to; list the nodes you
   connected by hand. Listing extras is free - `dispose()` runs once per node -
   so when in doubt, list everything you created.
4. **Expose the inlets with `Object.assign`.** An inlet is a `Param` node's
   `.input` when it needs scaling (a volume in decibels) or fan-out (one gate
   driving two envelopes). Otherwise expose the module's own `AudioParam`
   directly.
...
function Voice(ac: AudioContext, frequency = 440) {
  ...
  return Object.assign(
    disposable(out, [osc, filterEnv, filter, amp, gate, volume]),
    { gate: gate.input, volume: volume.input, osc, filter }
  );
}
```
