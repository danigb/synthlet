describe("DigitalDelayProcessor", () => {
  let Processor: any;
  ...
  beforeAll(async () => {
    ...
    Processor = (await import("./worklet")).DigitalDelayProcessor;
  });

  it("registers processor", () => {
    expect(global.registerProcessor).toHaveBeenCalledWith(
      "DigitalDelayProcessor",
      Processor,
    );
  });
  ...
  it("keeps producing output after the input goes silent", () => {
    ...
    const processor = new Processor();
    const settings = params({ time: 0.01, feedback: 0.7, mix: 1 });

    // 441 samples of delay, so ~4 blocks of input is well inside the line.
